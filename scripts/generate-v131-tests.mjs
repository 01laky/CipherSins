#!/usr/bin/env node
/**
 * Generates v1.3.1 exhaustive edge-case vitest suites (CS-V131-*).
 * Run: node scripts/generate-v131-tests.mjs
 */
...
function writeScanTestFile(area, cases, outFile, suppressionMode = false) {
	const expectBlock = suppressionMode
		? `expect(Object.keys(expected).length === 0 ? result.findings.length : countRule(result.findings, Object.keys(expected)[0])).toBe(Object.keys(expected).length === 0 ? 0 : expected[Object.keys(expected)[0]]);`
		: `
	...
	const tests = cases
		.map(
			(c) => `
	it("CS-V131-${area}-${c.id} ${c.label.replace(/"/g, "'")}", async () => {
		const result = await scanSource("case-${c.id}.ts", ${JSON.stringify(c.source)});
	...
	const content = `// Auto-generated by scripts/generate-v131-tests.mjs — do not edit by hand
import { describe, expect, it } from "vitest";
import { countRule, scanSource } from "../helpers/v131-scan-source.js";

describe("CS-V131 ${area} exhaustive edge cases", () => {${tests}
});
`;
...
function writeSuppressionTests(cases) {
	const tests = cases
		.map(
			(c) => `
	it("CS-V131-SUP-${c.id} ${c.label.replace(/"/g, "'")}", async () => {
		const result = await scanSource("sup-${c.id}.ts", ${JSON.stringify(c.source)}, ${c.allowCriticalIgnore ? "{ scan: { allowCriticalIgnore: true } }" : "undefined"});
		const ruleId = ${JSON.stringify(c.ruleId ?? "")};
		const expected = ${JSON.stringify(c.expect)};
	...
	const content = `// Auto-generated by scripts/generate-v131-tests.mjs
import { describe, expect, it } from "vitest";
import { countRule, scanSource } from "./helpers/v131-scan-source.js";

describe("CS-V131 suppressions exhaustive", () => {${tests}
});
`;

	fs.writeFileSync(
		path.join(root, "test/suppressions-v131-exhaustive.test.ts"),
		content,
	);
	console.log(`Wrote ${cases.length} suppression tests`);
}
...
function writeFixtureMatrixTests(fixtureCases) {
	const badLimitationFixtures = new Set([
...
describe("CS-V131 per-fixture file matrix", () => {${tests}
});
`;
	fs.writeFileSync(
		path.join(root, "test/cs-v131-fixture-matrix.test.ts"),
		content,
	);
...
function writeExpandedCliTests() {
	...
	for (const [slug, rel, ruleId] of badDirs) {
		...
		for (const fmt of formats) {
			tests.push(`
	it("CS-V131-CLI-${pad(n++)} ${slug} bad format ${fmt}", () => {
	...
	for (const rel of goodDirs) {
		for (const fmt of formats) {
			tests.push(`
	it("CS-V131-CLI-${pad(n++)} good ${rel.split("/").pop()} fmt ${fmt}", () => {
		const dir = path.join(rootDir, ${JSON.stringify(rel)});
...
describe("CS-V131 CLI exhaustive expanded", () => {${tests.join("")}
});
`;
	fs.writeFileSync(
		path.join(root, "test/cli/cs-v131-cli-exhaustive.test.ts"),
		content,
	);
...
function writeExpandedHelperUnitTests() {
	...
	for (const w of authWords) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} auth word ${w}", () => { expect(isAuthMaterialName("${w}")).toBe(true); });`,
		);
	}
	for (const w of camelAuth) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} camel auth ${w}", () => { expect(isAuthMaterialName("${w}")).toBe(true); });`,
		);
	}
	for (const w of nonAuth) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} non-auth ${w}", () => { expect(isAuthMaterialName("${w}")).toBe(false); });`,
		);
	}
	const weakHashes = [
		"md5",
	...
	for (const h of weakHashes) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} weak ${h}", () => { const e = parseExpr('createHash("${h}")'); expect(isWeakHashAlgorithmLiteral(e.arguments[0])).toBe(true); });`,
		);
	}
	for (const h of strongHashes) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} strong ${h}", () => { const e = parseExpr('createHash("${h}")'); expect(isWeakHashAlgorithmLiteral(e.arguments[0])).toBe(false); });`,
		);
	}
	const weakCiphers = [
		"des-cbc",
	...
	for (const c of weakCiphers) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} weak cipher ${c}", () => { const e = parseExpr('createCipheriv("${c}", k, iv)'); expect(isWeakCipherAlgorithmLiteral(e.arguments[0])).toBe(true); });`,
		);
	}
	const ecbCiphers = [
		"aes-128-ecb",
	...
	for (const c of ecbCiphers) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} ecb ${c}", () => { const e = parseExpr('createCipheriv("${c}", k, iv)'); expect(isEcbCipherAlgorithmLiteral(e.arguments[0])).toBe(true); });`,
		);
	}
	for (const cost of Array.from({ length: 16 }, (_, i) => i + 4)) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} bcrypt cost ${cost}", () => { const e = parseExpr('bcrypt.hashSync(password, ${cost})'); expect(isWeakBcryptCostLiteral(e.arguments[1])).toBe(${cost < 10}); });`,
		);
	}
	for (const iter of [0, 1, 999, 1000, 50000, 99999, 100000, 500000]) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} pbkdf2 ${iter}", () => { const e = parseExpr('pbkdf2Sync(p, s, ${iter}, 32, "sha256")'); expect(expressionIsLowPbkdf2IterationCount(e.arguments[2])).toBe(${iter < 100000}); });`,
		);
	}
	const camels = [
	...
	for (const [input, parts] of camels) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} split ${input}", () => { expect(splitCamelCase("${input}")).toEqual(${JSON.stringify(parts)}); });`,
		);
	}
	for (const len of [1, 4, 8, 15, 16, 17, 32, 64, 128]) {
		tests.push(
			`\n\tit("CS-V131-HLP-${pad(n++)} rng02 scan ${len}", async () => { const { scanSource, countRule } = await import("../helpers/v131-scan-source.js"); const r = await scanSource("x.ts", 'import { randomBytes } from "crypto";\\nexport function generateSessionToken() { return randomBytes(${len}); }\\n'); expect(countRule(r.findings, "CS-RNG-02")).toBe(${len < 16 ? 1 : 0}); });`,
		);
	}
	const content = `// Auto-generated by scripts/generate-v131-tests.mjs
...
describe("CS-V131 helper unit exhaustive", () => {${tests.join("")}
});
`;
	fs.writeFileSync(
		path.join(root, "test/rules/cs-v131-helpers-exhaustive.test.ts"),
		content,
	);
...
function writeExpandedScanEngineTests() {
	...
	for (const ext of extensions) {
		tests.push(`\n\tit("CS-V131-ENG-${pad(n++)} ext ${ext}", async () => {
	...
	for (const ruleId of allRules) {
		tests.push(`\n\tit("CS-V131-ENG-${pad(n++)} severity low ${ruleId}", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ciphersins-eng-"));
...
describe("CS-V131 scan engine exhaustive", () => {${tests.join("")}
});
`;
	fs.writeFileSync(
		path.join(root, "test/cs-v131-scan-engine.test.ts"),
		content,
	);
...
function writeExpandedReportingTests() {
	...
	for (const ruleId of allRules) {
		for (const fmt of ["json", "sarif"]) {
			tests.push(`\n\tit("CS-V131-REP-${pad(n++)} ${fmt} ${ruleId}", async () => {
...
describe("CS-V131 reporting exhaustive", () => {${tests.join("")}
});
`;
	fs.writeFileSync(
		path.join(root, "test/cs-v131-reporting-exhaustive.test.ts"),
		content,
	);
...
const jwtCases = multiplyJwt(
	renumberCases([...buildJwtCases(), ...buildMassiveJwtGrid()]),
);
const hashCases = multiplyHash(
	renumberCases([...buildHashCases(), ...buildMassiveHashGrid()]),
);
const encCases = multiplyHash(
	renumberCases([...buildEncCases(), ...buildMassiveEncGrid()]),
);
const cmpRngCases = multiplyHash(renumberCases(buildCmpRngCases()));
...
writeScanTestFile("JWT", jwtCases, "test/rules/cs-v131-jwt-exhaustive.test.ts");
writeScanTestFile(
	"HASH",
	hashCases,
	"test/rules/cs-v131-hash-exhaustive.test.ts",
);
writeScanTestFile("ENC", encCases, "test/rules/cs-v131-enc-exhaustive.test.ts");
writeScanTestFile(
	"CMP-RNG",
	cmpRngCases,
